...
<div class="relative flex w-full max-w-[26rem] flex-col rounded-xl bg-white bg-clip-border shadow-lg">
    <div
        ...
        <img src={{ $getItems()['image_note_path'] }} alt="ui/ux review check" />
        <div
                    ...
                    @foreach ($comments as $item)
                        <x-filament::dropdown.list>
                            <x-filament::dropdown.list.item
                                href="{{ route('filament.resources.notes.show-note-comment', ['record' => $getItems()['id']]) }}"
                                tag="a">
                                {{ $item['content'] }}
                            </x-filament::dropdown.list.item>
                        </x-filament::dropdown.list>
            ...
            <x-filament::dropdown>
                ...
                @if ($getItemsl()->count() != 0)
                    @foreach ($reactions as $item)
                        <x-filament::dropdown.list>
                            <x-filament::dropdown.list.item
                                href="{{ route('filament.resources.notes.show-likes', ['record' => $getItems()['id']]) }}"
                                tag="a">
                                {{ $item['mensaje'] }}
                            </x-filament::dropdown.list.item>
                        </x-filament::dropdown.list>
